...
int get_next_empty_element(void)
{
    ...
    {
       fprintf(stderr,"Line: %d : Compiler error! Symbol table overflow!", line);
        exit(EXIT_FAILURE);
    }
}
...
{
        return first_empty ? first_empty -1 : first_empty ; 
...
int insert_symbol(char *name, char *function, char *kind, char *type, int level, int class_id)
{
    ...
    symbol_table[index].param_index = 0;//nbre de param dans la fonction
...
void set_param(char *name, char *type)
{
    ...
    if (index < PARAM_NUMBER)
    {
        if (index != 0)
        {
            for (int i = 0; i < index; i++)
            {
                ...
                {
                    fprintf(stderr,"Line: %d : Compiler error! Method parameter ' %s ' already declared !", line, name);
                    exit(EXIT_FAILURE);
                }
            }
        }
    ...
    {
        fprintf(stderr,"Line: %d : Compiler error! Method parameters ' %s 'number overflow, max is %d !", line, function->name, PARAM_NUMBER);
        exit(EXIT_FAILURE);
    }
        print_symtab();
}
...
void insert_call_param(int f_index, char *val, char *type)
{
    ...
    if (strcmp(type, "IDENT") == 0)
    {
        ...
        if (test == 0)
        {
            fprintf(stderr,"Line: %d : Compiler error! Variable  %s  is not defined !", line, val);
            exit(EXIT_FAILURE);
        }
    }
    if (index < PARAM_NUMBER)
    ...
    {
        fprintf(stderr,"Line: %d : Compiler error! Method ' %s ' parameters number overflow, max is %d !", line, function->name, PARAM_NUMBER);
        exit(EXIT_FAILURE);
    }
}
...
int lookup_variable(char *name, char *type, int level, int class_id)
{
    if (level == 0)
    {
        ...
        while (strcmp(symbol_table[i].kind, "VARIABLE") == 0)
        {
            ...
            {
                fprintf(stderr,"Line: %d : Compiler error! Variable already declared in class scope!", line);
                exit(EXIT_FAILURE);
            }
            else
            {
    ...
    else if (level == 1)
    {
        ...
        while (strcmp(symbol_table[i].kind, "VARIABLE") == 0)
        {
            if (strcmp(symbol_table[i].name, name) == 0)
            {
               fprintf(stderr,"Line: %d : Compiler error! Variable already declared in method scope!", line);
                exit(EXIT_FAILURE);
            }
            else
                i--;
        ...
        for (int j = 0; j < function->param_index; j++)
        {
            ...
            {
               fprintf(stderr,"Line: %d : Compiler error! Variable already declared in method ' %s ' scope!", line, function->name);
                exit(EXIT_FAILURE);
            }
        }
    }
...
int lookup_class(char *name, int class_id)
{
    ...
    else
    {
        for (int i = first_empty - 1; i >= 0; i--)
        {
            ...
            {
               fprintf(stderr,"Line: %d : Compiler error! Class ' %s ' is already declared!", line, name);
                exit(EXIT_FAILURE);
            }
        }
        ...
        if (strcmp(symbol_table[i].kind, "METHOD") == 0 && strcmp(symbol_table[i].function, "DECLARATION") == 0)
        {
            ...
            for (int j = i + 1; j <= get_last_element_index(); j++)
            {
                if (strcmp(symbol_table[j].kind, "METHOD") == 0 && strcmp(symbol_table[j].function, "DECLARATION") == 0 && strcmp(symbol_table[i].name, symbol_table[j].name) == 0 && symbol_table[i].class_id == symbol_table[j].class_id)
                {
                    ...
                    {
                       fprintf(stderr,"Line: %d : Compiler error! Method ' %s ' is already declared in class scope!", dec2->line, dec2->name);
                        exit(EXIT_FAILURE);
                    }
                }
            }
...
int lookup_declarations(char *name , char *function, char *kind, int level, int class_id)
{
    ...
    if (i < 0)
    {
        fprintf(stderr, "Compiler error! Variable on Line: %d is not declared!\n", line);
        // fprintf(stderr, "Line: %d : Compiler error! Variable %s is not declared!\n", line, name);
    ...
    for (int i = 0; i <= get_last_element_index(); i++)
    {
        if (strcmp(symbol_table[i].kind, "METHOD") == 0 && strcmp(symbol_table[i].function, "USE") == 0)
        {
            ...
            if (strcmp(node->type, "NEW_IDENT") == 0)
            {
                ...
                if (test == 0)
                {
                   fprintf(stderr,"Line: %d : Compiler error! Method ' %s ' is not declared in class scope ' %s ' or does not have valide arguments !", call->line, call->name, node->name);
                    exit(EXIT_FAILURE);
                }
            }
            else
            {
                ...
                if (test == 0)
                {
                   fprintf(stderr,"Line: %d : Compiler error! Method ' %s ' is not declared or does not have valide arguments !", symbol_table[i].line, symbol_table[i].name);
                    exit(EXIT_FAILURE);
                }
            }
        }
...
void check_class_declared(void)
{
    for (int i = 0; i <= get_last_element_index(); i++)
    {
        ...
        if (strcmp(symbol_table[i].kind, "VARIABLE") == 0 && strcmp(symbol_table[i].function, "INSTANTIATION") == 0)
        {
            ...
            if (test == 0)
            {
               fprintf(stderr,"Line: %d : Compiler error! Class ' %s ' is not declared!", symbol_table[i].line, symbol_table[i].name);
                exit(EXIT_FAILURE);
            }
        }
    }
